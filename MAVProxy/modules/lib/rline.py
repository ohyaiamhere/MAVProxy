...
if __name__ == "__main__":
    ...
    class mystate(object):
        def __init__(self):
            ...
            [ MPSetting('foo', int, 1, tr("set_foo_int"), tab=tr("set_link"), range=(0,4), increment=1),
              MPSetting('bar', float, 4, tr("set_bar_float"), range=(-1,20), increment=1)])
            self.completions = {
                "script" : ["(FILENAME)"],
                "set"    : ["(SETTING)"]
                }
            self.command_map = {
                'script'  : (None,   tr("cmdhelp_script")),
                'set'     : (None,   tr("cmdhelp_set")),
                }
            self.aliases = {}
